...
* `url` (type: _string_, allowed: TCP, SSH, ICMP or HTTP URLs, default: empty) - Single Node URL to be probed (only used within the `group.node` spec and when `mode` is `poll`)
